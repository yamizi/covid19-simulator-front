...
class Covid19Form extends React.Component {
  ...
  handleSubmit = () => {
    var selectedLines = this.state.rows.filter(
      value => this.state.selectedIndexes.indexOf(value.id) !== -1
    )
    ...
    return (
      ...
      >
        {this.state.scenarios.map((v, i) => (
          <MenuItem key={i} id={v.id} onClick={this.handleScenarioClick}>
            <p>{v.id}</p>
          </MenuItem>
        ))}
      </Menu>
  ...
  data_to_save = name => {
    ...
    let data = this.state.rows.filter(
      value => this.state.selectedIndexes.indexOf(value.id) !== -1
    )
    ...
    //data.unshift({ country: this.state.countryName })
    return scenarios
  }
    ...
    return (
      <div>
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <HeaderAuthors>
              By Serval & Trux research groups @ SnT, University of Luxembourg
            </HeaderAuthors>
            <InputTutorial
          ...
          <Grid id="covid-form" item xs={5}>
              ...
              <Toolbar variant="dense">
                <div className={classes.left}>
                  <IconButton
                    ...
                    onClick={this.handleNewMeasureClick}
                    color="inherit"
                    aria-label="add"
                  >
                  ...
                  <IconButton
                    ...
                    edge="start"
                    className={classes.menuButton}
                    onClick={this.handleDeleteMeasureClick}
                    color="inherit"
                    aria-label="remove"
                  >
                    <RemoveCircleIcon />
            ...
            <LoadSaveDialog action={"save"} data_to_save={this.data_to_save} />
          </Grid>
          <Grid item xs={7}>
            {this.state.loading && (
              <div
            ...
            {this.state.data_json && !this.state.loading && (
              <Chart data={this.state.data_json} />
            )}
